...
async def get_all_operations(get_current_user:AuthJWT = Depends()):
...
async def edit_operation(name:str, restaurant:Restaurant, get_current_user:AuthJWT = Depends()):
    get_current_user.jwt_required()
...
async def delete_operation(name, get_current_user:AuthJWT = Depends()):
    get_current_user.jwt_required()
